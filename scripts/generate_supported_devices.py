...
SUPPORT_COLUMN_GROUPS = [
    ("Config", SUPPORT_COLUMNS[:4]),
    ("Control", SUPPORT_COLUMNS[4:]),
]
...
def support_matrix_markdown(rows: tuple[FirmwareSupportRow, ...]) -> str:
    ...
    for group, columns in SUPPORT_COLUMN_GROUPS:
        lines.append(f'      <th colspan="{len(columns)}">{html_escape(group)}</th>')
    ...
    for group, columns in SUPPORT_COLUMN_GROUPS:
        for _, _, label in columns:
            if group == "Config":
                label = label.removesuffix(" config")
            lines.append(f"      <th>{html_escape(label)}</th>")
    ...
    for row in rows:
        lines.append("    <tr>")
        lines.append(f"      <td>{html_escape(row.firmware)}</td>")
        for trait, _, _ in SUPPORT_COLUMNS:
            lines.append(f"      <td>{html_escape(row.support[trait])}</td>")
        lines.append("    </tr>")
...
def model_summary_markdown(makes: tuple[MakeModels, ...]) -> str:
    lines = [
    ...
    for make in makes:
        ...
        details_link = f"[Details](#{slugify(make.make)}-supported-models)"
        lines.append(
            "| "
            + " | ".join(
                [
                    table_escape(make.make),
                    f"{count} {model_label}",
                    table_escape(family_summary_text(families)),
                    details_link,
                ]
            )
            + " |"
...
def wrapped_inline_models(models: tuple[ModelEntry, ...], line_width: int = 120) -> str:
    ...
    for model in models:
        item = model_display(model)
        fragment = item if not current else f", {item}"
        if current and len(current) + len(fragment) > line_width:
...
def exact_models_markdown(makes: tuple[MakeModels, ...]) -> str:
    ...
    for make in makes:
        ...
        lines.extend(
            [
                f'<a id="{slugify(make.make)}-supported-models"></a>',
                "<details>",
                (
                    f"<summary>{make.make} ({count} {model_label} across "
                    f"{len(families)} {family_label})</summary>"
                ),
                "",
            ]
        )
        ...
        for family in families:
            ...
            lines.extend(
                [
                    "<details>",
                    (
                        f"<summary>{family.name} family "
                        f"({family_count} {family_model_label})</summary>"
                    ),
                    "",
    ...
    return (
        "\n\n".join(
            [
                ...
                (
                    "Legend: \u2705 means every backend subtype for that firmware type "
                    "supports the function; \U0001f7e1 means support is mixed or "
                    "conditional; \u274c means no backend subtype supports it."
                ),
                "## Support Matrix\n\n" + support_matrix_markdown(rows),
                "## Supported Models Summary\n\n" + model_summary_markdown(makes),
