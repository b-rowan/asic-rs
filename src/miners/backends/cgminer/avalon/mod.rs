...
impl AvalonMiner {
    ...
    pub async fn fault_light_on(&self) -> anyhow::Result<bool> {
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", "led", "1-1"]))).await?;
    ...
    pub async fn fault_light_off(&self) -> anyhow::Result<bool> {
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", "led", "1-0"]))).await?;
    ...
    pub async fn set_power_limit(&self, wattage: i32) -> anyhow::Result<bool> {
        ...
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", "worklevel,set", limit.to_string()]))).await?;
    ...
    pub async fn set_work_mode(&self, mode: u8) -> anyhow::Result<bool> {
        ...
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", format!("workmode,set,{}", mode)]))).await?;
    ...
    pub async fn soft_power_on(&self, timestamp: u64) -> anyhow::Result<bool> {
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", format!("softon,1:{}", timestamp)]))).await?;
    ...
    pub async fn soft_power_off(&self, timestamp: u64) -> anyhow::Result<bool> {
        let data = self.rpc.send_command("ascset", false, Some(json!(["0", format!("softoff,1:{}", timestamp)]))).await?;
    ...
    fn parse_stats(&self, stats: &str) -> HashMap<String, Vec<String>> {
        ...
        for cap in re.captures_iter(stats) {
            ...
            let values: Vec<String> = if value_str.contains(' ') {
                value_str.split_whitespace().map(|s| s.to_string()).collect()
            } else {
            ...
            if let Some(stats_array) = stats_value.as_array() {
                for stat in stats_array {
                    if let Some(id) = stat.get("ID").and_then(|v| v.as_str()) {
                        if id == "AVALON0" {
                            if let Some(mm_summary) = stat.get("MM ID0:Summary").and_then(|v| v.as_str()) {
                                let mm_stats = self.parse_stats(mm_summary);
                                
                                for board in 0..expected_hashboards {
                                    let board_idx = board as usize;
                                    
                                    if let Some(mghs) = mm_stats.get("MGHS").and_then(|v| v.get(0)) {
                                    ...
                                    if let Some(itemp) = mm_stats.get("ITemp").and_then(|v| v.get(0)) {
                                        if let Ok(temp) = itemp.parse::<f64>() {
                                            hashboards[board_idx].intake_temperature = Some(Temperature::from_celsius(temp));
                                        }
                                    }
                                    
                                    if let Some(hbi_temp) = mm_stats.get("HBITemp").and_then(|v| v.get(0)) {
                                        if let Ok(temp) = hbi_temp.parse::<f64>() {
                                            hashboards[board_idx].board_temperature = Some(Temperature::from_celsius(temp));
                                        }
                                    }
                            ...
                            if let Some(hb_info) = stat.get("HBinfo").and_then(|v| v.as_str()) {
                                ...
                                if let Some(pvt_temps) = hb_stats.get("PVT_T0") {
                                    if !pvt_temps.is_empty() {
                                        let working_chips = pvt_temps.iter()
                                            .filter(|temp| *temp != "0")
                                            .count();
    ...
    fn parse_expected_hashrate(&self, data: &HashMap<DataField, Value>) -> Option<HashRate> {
        if let Some(stats) = data.get(&DataField::ExpectedHashrate) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(ghsmm) = parsed_stats.get("GHSmm").and_then(|v| v.get(0)) {
    ...
    fn parse_fans(&self, data: &HashMap<DataField, Value>) -> Vec<FanData> {
        ...
        if let Some(stats) = data.get(&DataField::Fans) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(mm_id0) = parsed_stats.get("MM ID0").and_then(|v| v.get(0)) {
                    let mm_stats = self.parse_stats(mm_id0);
                    
                    for fan in 0..expected_fans {
                        let fan_key = format!("Fan{}", fan + 1);
                        if let Some(fan_speed) = mm_stats.get(&fan_key).and_then(|v| v.get(0)) {
        ...
        if let Some(stats) = data.get(&DataField::FluidTemperature) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(mm_id0) = parsed_stats.get("MM ID0").and_then(|v| v.get(0)) {
                    ...
                    if let Some(temp) = mm_stats.get("Temp").and_then(|v| v.get(0)) {
        ...
        if let Some(stats) = data.get(&DataField::Wattage) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(mm_id0) = parsed_stats.get("MM ID0").and_then(|v| v.get(0)) {
                    ...
                    if let Some(power) = mm_stats.get("WALLPOWER").and_then(|v| v.get(0)) {
        ...
        if let Some(stats) = data.get(&DataField::WattageLimit) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(mm_id0) = parsed_stats.get("MM ID0").and_then(|v| v.get(0)) {
                    ...
                    if let Some(power) = mm_stats.get("MPO").and_then(|v| v.get(0)) {
                        if let Ok(power_value) = power.parse::<f64>() {
                            return Some(Power::from_watts(power_value));
                        }
                    }
        ...
        if let Some(stats) = data.get(&DataField::LightFlashing) {
            if let Some(stats_str) = stats.as_str() {
                ...
                if let Some(mm_id0) = parsed_stats.get("MM ID0").and_then(|v| v.get(0)) {
                    ...
                    if let Some(led) = mm_stats.get("Led").and_then(|v| v.get(0)) {
        ...
        if let Some(pools_value) = data.get(&DataField::Pools) {
            if let Some(pools_array) = pools_value.as_array() {
                for (idx, pool) in pools_array.iter().enumerate() {
                    let url = pool.get("URL").and_then(|v| v.as_str()).map(|s| s.to_string());
                    let user = pool.get("User").and_then(|v| v.as_str()).map(|s| s.to_string());
                    let alive = pool.get("Status").and_then(|v| v.as_str()).map(|s| s == "Alive");
...
mod tests {
    ...
    fn create_mock_api_responses() -> HashMap<MinerCommand, Value> {
        ...
        results.insert(
            version_cmd,
            serde_json::from_str(VERSION_COMMAND).unwrap(),
        );
        ...
        results.insert(
            stats_cmd,
            serde_json::from_str(STATS_COMMAND).unwrap(),
        );
        ...
        results.insert(
            devs_cmd,
            serde_json::from_str(DEVS_COMMAND).unwrap(),
        );
        ...
        results.insert(
            pools_cmd,
            serde_json::from_str(POOLS_COMMAND).unwrap(),
        );
        ...
        results.insert(
            summary_cmd,
            serde_json::from_str(SUMMARY_COMMAND).unwrap(),
        );
    ...
    async fn test_avalon_hashboard_parsing() {
        ...
        assert!(!miner_data.hashboards.is_empty(), "Hashboards should be populated");
        
        let first_board = &miner_data.hashboards[0];
        assert_eq!(first_board.position, 0);
        assert!(first_board.active.unwrap_or(false), "Board should be marked as active");
        
        assert!(first_board.board_temperature.is_some() || first_board.intake_temperature.is_some(),
    ...
    async fn test_avalon_temperature_parsing() {
        ...
        if let Some(temp) = &miner_data.fluid_temperature {
            assert!(temp.as_celsius() > -50.0 && temp.as_celsius() < 150.0, 
    ...
    async fn test_avalon_stats_parsing() {
        ...
        assert!(parsed.contains_key("ITemp"), "Should parse internal temperature");
        assert!(parsed.contains_key("Fan1"), "Should parse fan data");
        
        // Test specific values
        if let Some(ghsmm) = parsed.get("GHSmm").and_then(|v| v.get(0)) {
            assert_eq!(ghsmm, "55032.79", "GHSmm should match expected value");
        }
        
        if let Some(itemp) = parsed.get("ITemp").and_then(|v| v.get(0)) {
